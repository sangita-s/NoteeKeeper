...
public class DatabaseDataWorker {
    ...
    public void insertSampleNotes() {
        insertNote("android_intents", "DBDynamic intent resolution", "Wow, intents allow components to be resolved at runtime");
        insertNote("android_intents", "DBDelegating intents", "PendingIntents are powerful; they delegate much more than just a component invocation");

        insertNote("android_async", "DBService default threads", "Did you know that by default an Android Service will tie up the UI thread?");
        insertNote("android_async", "DBLong running operations", "Foreground Services can be tied to a notification icon");

        insertNote("java_lang", "DBParameters", "Leverage variable-length parameter lists?");
        insertNote("java_lang", "DBAnonymous classes", "Anonymous classes simplify implementing one-use types");

        insertNote("java_core", "DBCompiler options", "The -jar option isn't compatible with with the -cp option");
        insertNote("java_core", "DBSerialization", "Remember to include SerialVersionUID to assure version compatibility");
    }
